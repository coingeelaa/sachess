...
async function transferFARASbot(bonusAmount, userPublicKey) {
  try {
    // Haddii FARASbot uu leeyahay 9 decimals:
    const decimals = 9;
    const integerAmount = Math.round(bonusAmount * 10 ** decimals);

    // Hel ama samee token account-ka bot-ka
    ...
    // Hel ama samee token account-ka user-ka
...
const serviceAccount = require("./saches-4dc78-firebase-adminsdk-fbsvc-73c2689717.json");
admin.initializeApp({
...
async function updateReferralBonus(referrerCode, feePaid, transactionData, referredUserId) {
  try {
    ...
    let bonusPercentage = 0.10; // default
    if (monthDiff === 0) {
...
// Get referral stats: code, link, referralsCount, lifetimeBonk
...
// ----------------- Helper Functions for Solana (Price, Valid Address, Save TX, etc.) -----------------
...
async function importWalletByPrivateKey(userId, phone, firstName, lastName, username, email, privateKeyInput) {
  try {
    ...
    if (/^[0-9a-fA-F]+$/.test(privateKeyInput)) {
      ...
      secretKeyUint8 = bs58.decode(privateKeyInput);
    }
...
async function recoverWalletByPhrase(userId, phone, firstName, lastName, username, email, phrase) {
  try {
    // In real usage, you'd derive a keypair from the mnemonic phrase
    // but for demonstration, we just create a new wallet
...
bot.action('referral_friends', async (ctx) => {
  try {
    ...
    // --- FIXED HERE: Use newlines instead of <br> ---
    const messageText = `
    ...
    if (ctx.session.cashBuy) {
      ...
      // Step 2: amount
        ...
        if (isNaN(amount) || amount <= 0) {
          await ctx.reply('❌ Invalid amount. Please enter a valid number.', { parse_mode: 'HTML' });
          return;
        }
...
bot.action('confirm_send', async (ctx) => {
  try {
    ...
    if (activeWallet.type === 'import') {
      fromKeypair = Keypair.fromSecretKey(bs58.decode(storedPrivateKey));
    } else {
...
async function placeMarketOrder(usdtAmount) {
  ...
  // Simulate a response object for demonstration
  return {
    data: {
      symbol: "SOLUSDT",
      orderId: 123456,
      executedQty: (usdtAmount / 22).toFixed(6), // pretend 1 SOL = $22
